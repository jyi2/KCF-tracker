function [start_frame, img_files1, img_files2, pos, target_sz, ground_truth, video_path] = load_video_info(start_frame, base_path, video)
%LOAD_VIDEO_INFO
...
%
%	img_files1 is the frames before the start_frame(included)
%	img_files2 os the frames after the start_frame(included)
%	pos is the centroid position (y, x) of the bounding box
%	target_sz is the (height, width) of the bounding box
%	ground_truth is a Nx2 matrix from groundtruth_rect.txt, but format is changed to centroid positions
%	video_path is the path of the video folder
	...
	if isempty(idx),
		...
		num = size(img_files,2);
		%adjust start_frame for videos that don't keep every frame
		frame = num2str(start_frame);
		indexC = strfind(img_files, frame);
		index = find(not(cellfun('isempty', indexC)));
		start_frame = index(1);
		img_files1 = img_files(1:index);
		img_files2 = img_files(index:num);
		display(start_frame);
	else
	...
	target_sz = [ground_truth(start_frame,4), ground_truth(start_frame,3)];
	display(target_sz);
	pos = [ground_truth(start_frame,2), ground_truth(start_frame,1)] + floor(target_sz/2);
	display(pos);
